...
Logger = LoggerStatic = class Logger {
  ...
  static isActive() {
    return LoggerStatic.contextEnvironment !== Environment.TEST
  }
  ...
  static printMessage(message, color, context = '', isTimeDiffEnabled) {
    ...
    const output = SharedUtils.isObject(message)
      ...
      : message
    process.stdout.write(color(`[Application] ${process.pid}   - `))
  ...
  static printTimestamp(isTimeDiffEnabled) {
    ...
    if (includeTimestamp) {
      process.stdout.write(
        this.yellow(` +${Date.now() - LoggerStatic.prevTimestamp}ms`)
      )
    }
...
Logger.contextEnvironment = Environment.RUN
